...
class WMSFaissBuilder:
    """JSON 청크 파일들을 Faiss 벡터 데이터베이스로 변환하는 메인 클래스입니다."""
    ...
    def setup_directories(self):
        ...
        self.logger.info(f"Faiss DB 디렉토리 준비: {self.faiss_storage_dir}")
    ...
    def build_vector_database(self):
        ...
        self.logger.info("🚀 Faiss 벡터 데이터베이스 구축 시작...")
        
        # 기존 데이터 확인
        index_path = self.faiss_storage_dir / "wms_knowledge.index"
        ...
        if all_embeddings:
            ...
            # 최종 통계
            self.logger.info("=" * 60)
            self.logger.info("🎉 Faiss 벡터 데이터베이스 구축 완료!")
    ...
    def test_search(self, query: str = "창고 자동화", top_k: int = 5):
        """벡터 검색 테스트를 수행합니다."""
...
def main():
    """메인 실행 함수"""
    parser = argparse.ArgumentParser(description="WMS Faiss 구축기")
