...
class WMSChromaDBBuilder:
    ...
    def setup_embedding_model(self):
        ...
        if self.embedding_model_type == "sentence-transformers":
            ...
            # 본 프로젝트 호환성을 위한 임베딩 모델 선택
            # 기본: 빠르고 효율적 (384차원)
            model_name = "sentence-transformers/all-MiniLM-L6-v2"  
            
            # 고품질 옵션 (768차원) - 본 프로젝트가 높은 차원을 지원하는 경우
            # model_name = "sentence-transformers/all-mpnet-base-v2"
            
            # OpenAI 호환 옵션 (1536차원과 유사한 성능)
            # model_name = "sentence-transformers/all-MiniLM-L12-v2"
            
            self.logger.info(f"선택된 모델: {model_name} (차원: 384)")
            self.logger.info("💡 본 프로젝트 호환성을 위해 차원 확인 필요")
