    ...
    def export_network_data(self):
        ...
        try:
            if len(self.citation_graph.nodes) > 0:
                ...
                self.logger.info("[SUCCESS] Citation network GEXF  ")
        except Exception as e:
            self.logger.error(f"[ERROR] Citation network GEXF  : {e}")
        ...
        try:
            if len(self.similarity_graph.nodes) > 0:
                ...
                self.logger.info("[SUCCESS] Similarity network GEXF  ")
        except Exception as e:
            self.logger.error(f"[ERROR] Similarity network GEXF  : {e}")
